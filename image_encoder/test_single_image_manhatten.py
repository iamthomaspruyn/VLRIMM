...
def find_and_plot_matches(target_emb, db_embeddings, db_paths, top_k=10, bottom_k=10):
    ...
    # CHANGED: Use manhattan_distances instead of cosine_similarity
    # Note: Output is Distance (Lower is better), not Similarity (Higher is better)
    dists = manhattan_distances(target_emb, db_embeddings)[0]

    # --- 1. Get Indices ---

    # Top K: Smallest distances (Best Matches)
    # argsort is ascending (small to large). We take the first K.
    top_indices = np.argsort(dists)[:top_k]

    # Bottom K: Largest distances (Worst Matches)
    # We take the last K, then reverse to have the very largest first.
    bottom_indices = np.argsort(dists)[-bottom_k:][::-1]

    # --- 2. Visualization ---
    print(f"Plotting Top {top_k} and Bottom {bottom_k} matches based on Manhattan Distance...")
    ...
    for i, idx in enumerate(top_indices):
        ...
        if col < total_cols:
            score = dists[idx]
            path = db_paths[idx]
            # CHANGED: Label is now "Dist"
            title = f"Top #{i + 1}\nDist: {score:.4f}"
            # Green box if extremely close (arbitrary threshold for L1, adjusted to strict check or removed)
            plot_image_on_ax(axes[0, col], path, title, box_color='green' if score < 1.0 else None)
    ...
    plt.tight_layout()
    output_file = "similarity_results_manhattan.png"
